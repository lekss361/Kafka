...
//Этот код был был перемещен из другой области. Изменение сейчас не рационально.
...
namespace Ekassir.KafkaFlow.Extensions
{
    ...
    internal abstract class WildcardPatternParser
    {
        ...
        internal static WildcardPatternException NewWildcardPatternException(string invalidPattern)
        {
            WildcardPatternException e =
                new WildcardPatternException($"WildcardPattern_Invalid: {invalidPattern}");
    ...
    internal class WildcardPatternMatcher
    {
        ...
        private sealed class PatternPositionsVisitor : IDisposable
        {
            ...
            {
                Debug.Assert(lengthOfPattern >= 0, "Caller should verify lengthOfPattern >= 0");
            ...
            {
                Debug.Assert(patternPosition >= 0, "Caller should verify patternPosition >= 0");
                Debug.Assert(
        ...
        private sealed class BracketExpressionElement : QuestionMarkElement
        {
            ...
            {
                Debug.Assert(regex != null, "Caller should verify regex != null");
                _regex = regex;
            }
        ...
        private sealed class MyWildcardPatternParser : WildcardPatternParser
        {
            ...
            {
                _bracketExpressionBuilder.Append(']');
                Regex regex = NewRegex(_bracketExpressionBuilder.ToString(), _regexOptions);
